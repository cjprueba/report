	...
	<div>
		...
		<div class="card shadow mb-4">
	       ...
	       <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
	         <h6 class="m-0 font-weight-bold text-primary">{{title}} <small>- Categoria - {{fecha}}</small></h6>
	       ...
	       <div class="card-body">
				...
				<table v-else class="table table-sm table-striped">
				  ...
				  <tbody>
				    <tr v-for="(categoria, index) in response">
				      <th scope="row">{{index+1}}</th>
				      <td>{{categoria.LINEA_NOMBRE}}</td>
				      <td>{{new Intl.NumberFormat("de-DE", {style: "decimal", decimal: "0"}).format(categoria.STOCK_G)}}</td>
				      <td>
				      	<div class="progress">
		  					<div class="progress-bar" role="progressbar" v-bind:style="{ width: categoria.P_STOCK + '%' }"   v-bind:aria-valuenow="categoria.P_STOCK" aria-valuemax="100">
		  						{{categoria.P_STOCK}}%
		  					</div>
						</div>
					  </td>
					  <td>{{new Intl.NumberFormat("de-DE", {style: "decimal", decimal: "0"}).format(categoria.PRECIO)}}</td>
				      <td>
				      	<div class="progress">
		  					<div class="progress-bar" role="progressbar" v-bind:style="{ width: categoria.P_TOTAL + '%' }"   aria-valuenow="categoria.P_STOCK" aria-valuemin="0" aria-valuemax="100">
		  						{{categoria.P_TOTAL}}%
		  					</div>
						</div>
					  </td>
					  <td>{{new Intl.NumberFormat("de-DE", {style: "decimal", decimal: "0"}).format(categoria.PRECIO_ANTERIOR)}}</td>
					  <td>
					  	<div v-bind:class="classObject(categoria.COMPORTAMIENTO_PRECIO)">
					  	{{categoria.COMPORTAMIENTO_PRECIO}}
					  		<font-awesome-icon v-if="caret(categoria.COMPORTAMIENTO_PRECIO)" icon="caret-up"/>
				      ...
				      <td>
				      	<div class="progress">
		  					<div class="progress-bar" role="progressbar" v-bind:style="{ width: categoria.P_VENDIDO + '%' }"   aria-valuenow="categoria.P_STOCK" aria-valuemin="0" aria-valuemax="100">
		  						{{categoria.P_VENDIDO}}%
		  					</div>
						</div>
					  </td>
					  <td>{{new Intl.NumberFormat("de-DE", {style: "decimal", decimal: "0"}).format(categoria.VENDIDO_ANTERIOR)}}</td>
					  <td>
					  	<div v-bind:class="classObject(categoria.COMPORTAMIENTO_VENDIDO)">
					  	{{categoria.COMPORTAMIENTO_VENDIDO}}
					  		<font-awesome-icon v-if="caret(categoria.COMPORTAMIENTO_VENDIDO)" icon="caret-up"/>
                        	<font-awesome-icon v-else icon="caret-down"/> 
                        </div> 
					  </td>
				    </tr>
        ...
        methods: {
	        llamarDatos(opcion){
				...
				axios.post('/ventas', this.datos).then(function (response) {
						me.response = response.data.categorias;
				});
